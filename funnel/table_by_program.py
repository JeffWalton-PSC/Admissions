...
tab_df = pd.concat([tab_df,  pd.DataFrame({last_yearterm+'_Week'+str(week) : summ.loc[(last_yearterm,)][str(week)]})],
                    axis = 1,
                    sort=True)
tab_df = pd.concat([tab_df,  pd.DataFrame({twoyrago_yearterm+'_Week'+str(week) : summ.loc[(twoyrago_yearterm,)][str(week)]})],
                    axis = 1,
                    sort=True)
tab_df = pd.concat([tab_df,  pd.DataFrame({last_yearterm+'_Total' : summ.loc[(last_yearterm,)]['53']})],
                    axis = 1,
                    sort=True)
tab_df = pd.concat([tab_df,  pd.DataFrame({twoyrago_yearterm+'_Total' : summ.loc[(twoyrago_yearterm,)]['53']})],
                    axis = 1,
                    sort=True)
