...
data_path = Path(r"F:\Applications\Admissions\funnel")
...
adm_df = pd.concat([stage_data, applied, accepted])

adm_df = adm_df.loc[
    (
        (adm_df["ACADEMIC_TERM"].isin(["FALL", "SPRING"]))
        & (adm_df["ACADEMIC_SESSION"] == "MAIN")
        & (adm_df["ACADEMIC_YEAR"] >= begin_year)
    )
]
...
def fill_weeks(field, data_frame):
    ...
    for w in weeks:
        # f = f"{w:02d}"
        # r.loc[:, f"{w:02d}"] = f_status(field, data_frame, w)
        r[f"{w:02d}"] = f_status(field, data_frame, w)
        r["stage"] = field
